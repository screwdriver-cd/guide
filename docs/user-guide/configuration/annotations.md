...
Pipeline-level annotations are used to modify the properties of the entire pipeline. Pipeline-level annotations are under the same level as `shared` and `jobs`
 ...
 | screwdriver.cd/chainPR    | `false` / `true` | Default value is `false`. When value is `false` a Pull Request will run only those jobs which has `~pr` in `requires`. When `true` pull request workflow will behave like regular commit workflow. Workflow will start at jobs which has `~pr` in requires and will also trigger any downstream jobs in the workflow.|
