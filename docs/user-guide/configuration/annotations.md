...
| (DEPRECATED) screwdriver.cd/diskSpeed | `TURBO` | When using a `k8s-vm` executor, this will allow the user to choose between machines with different disk speed. Default is `TURBO` which is the only disk speed SDv4 now provides (as of 08/30/2019). |
