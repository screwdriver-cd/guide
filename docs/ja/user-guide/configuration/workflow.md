...
ワークフローの順序を示すために、ジョブ設定で `requires` をジョブ名の配列で定義します。ジョブ名にチルダのプレフィックスを付けることで、[高度な設定 (Advanced Logic)](#高度な設定)を定義することができます。
...
## 高度な設定 (Advanced Logic)

### 高度な設定 [*AND*] (Advanced Logic [*AND*])
...
### 高度な設定 [*AND, OR の併用*] (Advanced Logic [*Combined*])
