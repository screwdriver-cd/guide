...
ワークフローは個々のジョブをパイプラインという形で紐づけてまとめる方法です。ジョブ定義の中で `requires` というキーワードで、ジョブと、ジョブ実行の契機となるイベントのリストを指定することで実現しています。
...
ワークフローの順序を示すために、ジョブ設定で `requires` をジョブ名の配列で定義します。ジョブ名には[高度なロジック](#高度なロジック)を示すために、チルダのプレフィックスがついている場合があります。
...
`requires` に含まれるジョブが  [*AND*] のロジックとして成功した場合に、特定のジョブを開始することができます。これはしばしばジョインやファン・インとも呼ばれます。
...
1つの require から2つ以上のジョブを並列に実行することができます。並列実行したジョブを1つのジョブに結合するには、複数のジョブを `requires` に設定します。
...
他のパイプラインの実行完了を契機に、パイプラインのジョブを実行することができます。 フォーマットは `~sd@pipelineID:jobName` です。`~pr` と `~commit` と `~sd@pipelineID:jobName` のフォーマットは *OR* のロジックに従います。
...
どこからもトリガーされないワークフローを定義することができます。これらのワークフローはパイプラインの通常のフローから"分離されています"。ユースケースとしては、手動でトリガーを行うロールバックのフローなどです。分離されたパイプラインの実行は、[ロールバック](../FAQ#ビルドのロールバックを行うには) と同じ手順です。
