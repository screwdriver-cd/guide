...
| screwdriver.cd/jobDisabledByDefault | `true` / `false` | 定期的にビルドが実行されているパイプラインがfork・コピーされた際に、fork先でビルドが定期的に実行されることを防ぎたい場合があります。このアノテーションを`true`に設定すると、作成されたジョブははじめ`disabled`となます。ユーザはUIからジョブを`enabled`にできます。デフォルトは`false`です。 |
